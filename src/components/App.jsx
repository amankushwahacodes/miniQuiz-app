...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'restart':
      return {
        ...intitalState, questions: state.questions, status: 'ready', highscore: state.highscore
      }
    case 'tick':
       return {
        ...state , secondsRemaining : state.secondsRemaining-1,
         highscore: state.secondsRemaining === 0 ? (state.points > state.highscore ? state.points : state.highscore) : null,
...
export default function App() {
  const [{ questions, status, index, answer, points, highscore,secondsRemaining }, dispatch] = useReducer(reducer, intitalState);
